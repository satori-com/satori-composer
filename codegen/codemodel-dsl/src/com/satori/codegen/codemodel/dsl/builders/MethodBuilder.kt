...
open class MethodBuilder(val name: String, val jdef: JDefinedClass) : BaseBuilder(jdef.owner()), IMethodScope, IHasAccessModifierTrait, IHasAnnotationsTrait {
  
  val args = ArrayList<ArgSpec>()
  var returns: JType = jmodel.VOID
  ...
  fun build() {
    val jmethod = jdef.method(mods, returns, name)
    args.forEach {
